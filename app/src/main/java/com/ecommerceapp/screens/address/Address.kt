...
fun AddressScreen(navController: NavHostController) {
    Surface(
        ...
        Column(
            ...
            ScreenGreeting(name = stringResource(id = R.string.address)) {
                navController.popBackStack()
            }

            Divider(modifier = Modifier.height(12.dp))
            AppButton(buttonname = stringResource(id = R.string.timeslot)) {
                navController.navigate(AppScreenName.TIMESLOT.name)
            }
        }
    }
}